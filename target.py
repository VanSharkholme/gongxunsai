...
def filter(frame, area_proportion, min_area, hw_proportion, color_frame):
    ...
    for contour in contours_frame:
        ...
        if (rect_area / con_area < area_proportion
            ...
            cv.rectangle(color_frame, rect, (0, 255, 0), -1)
    ...
    # if len(valid_box) > 0:
    #     cv.drawContours(color_frame, valid_box, -1, (0, 255, 0))
...
try:
    while True:
        ...
        hori = cv.morphologyEx(hori, cv.MORPH_OPEN, kernel1, iterations=9)
        hori = cv.morphologyEx(hori, cv.MORPH_DILATE, kernel1, iterations=10)
        hori = cv.blur(hori, (9, 3), )
        ...
        verti = cv.morphologyEx(verti, cv.MORPH_DILATE, kernel2, iterations=10)
        verti = cv.blur(verti, (3, 9), )
        hori_boxes = filter(hori, 500, 10, 1, color_frame)
        verti_boxes = filter(verti, 500, 10, 1, color_frame)
        centers = []
